...
module.exports = Sequencer.extend({
  ...
  //   var self = this;
  //
  //   // HERE IS WHERE THE FORK HAPPENS? OR ALREADY WHEN index is exported?
  //
  ...
  'logAppStatus': function (status) {

    var self = this;
    var pkg = self.config('PKG');
    var message = status + ' "' + pkg.name + '" @ ' + pkg.version;

    self.options.muteLog || self.logger.pending(self.namespace, message);
  },
