...
module.exports = Base.extend({
  ...
  'logRunning': function (display) {
    ...
    if (display) {
      ...
      var lastRequestCount = self.lastRequestCount || 0;
      self.lastRequestCount = self.requestCount;
      var diff = self.requestCount - lastRequestCount;
      var throughput = Math.round((diff / timeout) * 60);

      self.logger.log(self.namespace, 'Running ' + time.trim() + ', received ' + numberWithCommas(self.requestCount) + ' requests, sent ' + numberWithCommas(self.requestCount - self.errorCount) + ' non-error responses, throughput ' + numberWithCommas(throughput) + ' requests per minute');
    }
