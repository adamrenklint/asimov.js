define([
...
], function (Base, _, requirejs, zombie, Mocha, uri, npath, Loader, createUID) {
  ...
  return Base.extend({
    ...
    'integration': function (url, callback) {
      ...
      if (url.indexOf('http') < 0 && !self.server) {

        self.child.execute('node main.js');
        var loaded = self.deferred();
        ...
        // .done(function () {
        //   console.log('child done');process.exit(1)
        // });
        // var loader = new Loader(_.merge({}, self.options, {
        //   'muteLog': true
        // }));
        ...
        // start the main asimov process

        // then add localhost:3009 to the url
    ...
    'itShouldThrowError': function (message, callback) {
    ...
    'itShouldNotThrowError': function (message, callback) {
