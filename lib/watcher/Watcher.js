...
define([
  ...
  return Model.extend({
    ...
    'handleChange': function (path, before, after, type) {
      ...
      self.options.logVerbose && handler && self.logger.pending(self.namespace, 'A ' + pathType + ' file was ' + type + ', invalidating ' + graph.length + ' dependencies @ ' + path);
    ...
    'parseDependencies': function (model) {
      ...
      if (parser) {
        ...
        self.options.logVerbose && self.options.logVerbose && self.logger.since(self.namespace, 'Parsed dependencies @ ' + model.attributes.path, started);
