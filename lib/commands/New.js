...
module.exports = Command.extend({
  ...
  'template': function () {
    ...
    var self = this;
    var typeIndex = self.options.args.indexOf('template');
    var name = self.options.args[typeIndex + 1];

    self.assert('string', name, function () {
      self.error(['Invalid template name: ' + name]);
    });

    var outputFolder = npath.join(process.cwd(), 'site/templates');
    var outputPath = npath.join(outputFolder, name + '.tmpl');
    ...
    self.logger.since(self.namespace, 'Created new template @ ' + outputPath, self.started);
  },
