...
module.exports = Command.extend({
    ...
    if (self.filesystem.pathExists(path)) {
      ...
      self.renamePath(path).done(function (path) {
        self.overwriteWithTemplates(path).done(function () {
          ...
          self.copyModulesFolder(path).done(function () {
  ...
  'copyModulesFolder': function (path) {

    var self = this;
    var deferred = self.deferred();

    var source = npath.join(__dirname, '../../node_modules');
    var destination = npath.join(path, 'node_modules');
