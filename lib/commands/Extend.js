...
module.exports = Command.extend({
  ...
  'createClass': function (parent, path) {
    ...
    var parentPath = self.filesystem.findFirstMatch(parent + '.js', self.options.paths.scripts);
  ...
  'createTestForClass': function (parentPath, outputPath) {
    ...
    var output = template.attributes.compiled({
      'name': name,
      'path': npath.relative(self.getFolder(testPath), outputPath).replace('.js', '')
    });
  ...
  'getOutput': function (parentPath, outputPath) {
    ...
    return template.attributes.compiled({
      'parent': {
        'name': parentName,
        'path': npath.relative(self.getFolder(parentPath), outputPath).replace('.js', '')
      }
    });
  },
