...
module.exports = Base.extend({
  ...
  'run': function (model) {
    ...
    if (process.env.ENV !== 'production') {
      result = result.replace('</head>', '<script>document.write(\'<script src="http://\' + (location.host || \'localhost\').split(\':\')[0] + \':' + self.options.reloadPort + '/livereload.js"></\' + \'script>\')</script></head>');
    }
