...
define([
  ...
  return Base.extend({
    ...
    'render': function () {
      ...
      self.regulateRenderSize(start, end);
    },

    'regulateRenderSize': function (start, end) {

      var self = this;
      var diff = (end - start) * 2;
      var breakpoint = self.delay / 2;

      if (diff < breakpoint) {
        self.limit = Math.round(self.limit * 2) || 1;
      }
      else {
        self.limit = Math.round(self.limit / 2) || 1;
      }
