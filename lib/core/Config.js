...
define([
  ...
  return Base.extend({
    ...
    'logLoading': function (done, path, logger) {

      var self = this;

      if (!done || !logger) {
        logger = self.logger.wait(self.namespace, 'Loading environment config @ ' + path, true);
      }
      else {
        logger.nextAndDone();
        logger = null;
    ...
    'readConfig': function (path) {
      ...
      var logger = self.logLoading(false, path);
      ...
      if (self.filesystem.pathExists(path)) {
        ...
        self.logLoading(true, path, logger);
