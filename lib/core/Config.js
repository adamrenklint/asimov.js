...
define([
  ...
  return Base.extend({
    ...
    'generate': function () {
      ...
      var config = _.merge(self.options, self.filesystem.readJSON(frameworkConfigPath));

      self.logLoading(true, frameworkConfigPath);

      var environment = 'common';
      var configPath = self.filesystem.join(config.paths.configs, environment + '.json');

      self.logLoading(false, configPath);

      config = _.merge(config, self.filesystem.readJSON(configPath));
