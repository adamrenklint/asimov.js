...
], function (Base, Config, PageNodesCollection, StyleSheetNodesCollection, SiteDataCollection, Watcher, Queue, OutputWriter, TemplatesCollection, TemplateHelpersCollection, Server, Signature, _) {
  ...
  return Base.extend({
    ...
    'bootstrap': function () {
      ...
      self.when(loadedTemplates, loadedSiteData, loadedHelpers).done(function () {
        ...
        pages.fetch(config.json.paths.content).done(pages.ensureErrorPages);
      });
    }
  });
});