...
define([
  ...
  return Base.extend({
    ...
    'bootstrap': function () {
      ...
      // var loadedTemplates = (new TemplatesCollection(null, config.json)).fetch(config.json);
      // var loadedTemplateHelpers = (new TemplateHelpersCollection()).fetch(config.json);

      self.when(loadedPageNodes).done(function (collections) {

        var pages = collections[0];
