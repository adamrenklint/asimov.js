...
define([
...
], function (Base, Config, Cache, PageNodesCollection, SiteDataCollection, Watcher, Queue, TemplatesCollection, TemplateHelpersCollection, Server, Signature, _) {
  ...
  return Base.extend({
    ...
    'bootstrap': function () {
      ...
      var pages = new PageNodesCollection(null, config.json);
      ...
      self.when(loadedTemplates, loadedSiteData, loadedHelpers).done(function () {
        ...
        self.bindTo(pages, 'add', queue.add);
        self.bindTo(pages, 'change:raw', function () {
          throw new Error('GOOD, change:raw was triggered');
        })
        ...
        // WHY THE HELL IS THE CHANGE:RAW EVENT NEVER FIRED ON COLLECTIONS!?!?!?!?!
