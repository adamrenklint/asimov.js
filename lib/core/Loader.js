...
], function (Base, Config, PageNodesCollection, StyleSheetNodesCollection, SiteDataCollection, Watcher, Queue, OutputWriter, LastModifiedTracker, TemplatesCollection, TemplateHelpersCollection, Server, _) {
    ...
    'bootstrap': function () {
      ...
      self.when(loadedTemplates, loadedSiteData, loadedHelpers).done(function () {
        ...
        self.bindOnceTo(queue, 'empty', function () {
          self.logger.since(self.namespace, 'Started project "' + meta.name + '" in $', started);
        });
