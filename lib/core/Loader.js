...
define([
...
], function (Base, Config, Cache, PageNodesCollection, Watcher, Queue, Templates, TemplateHelpers, Server, Signature) {
  ...
  return Base.extend({
    ...
    'bootstrap': function () {
      ...
      // var pageNodes = new PageNodesCollection();
      // var crawled = pageNodes.fetch(config.json.paths.content);

      // crawled.done(function () {

      // });

      var loadedPageNodes = (new PageNodesCollection()).fetch(config.json.paths.content);
      // var loadedTemplates = (new TemplatesCollection()).fetch(config.json);
      // var loadedTemplateHelpers = (new TemplateHelpersCollection()).fetch(config.json);

      // self.when(loadedPageNodes, loadedTemplates, loadedTemplateHelpers).done(function (pages, templates, helpers) {

      //   console.log(pages, templates, helpers);process.exit(1)
      // });
