...
define([
  ...
  return MetaNode.extend({
    ...
    'initialize': function (attributes, options) {
      ...
      self.assert('defined', self.attributes.path, 'Cannot create page node without path');
      self.assert('string', self.attributes.path, 'Cannot create page node without path');

      if (self.attributes.path.indexOf('.txt') < 0) {
