...
define([
...
], function (Base, PageNode, Filesystem, _) {
  ...
  return Base.extend({
    ...
    'readNode': function (path) {
      ...
      // And the children
      filesystem.readDirectory(path, self.readNode);
      ...
      _.defer(function () {
        ...
        filesystem.watchTree(path, function (changed) {
          ...
          if (typeof changed === 'string') {
            ...
            var url = _.initial(changed.replace(path, '').split('/')).join('/') || '/';
            var node = self.map[url];

            if (node) {
              node.updateMetaNode(changed);
            }
            else {
              throw new Error('Failed to find existing page node @ ' + url);
            }
          }
          else {
